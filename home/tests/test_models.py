...
def page_tree(home_page):
    ...
    # /first
    first_page = PageFactory(depth=3, path='000100010001', slug='first', numchild=2, show_in_menus=True)

    # /first/first
    PageFactory(depth=4, path=f'{first_page.path}0001', slug='first', show_in_menus=True)
    # /first/second
    PageFactory(depth=4, path=f'{first_page.path}0002', slug='second', show_in_menus=True)
    ...
    # /unicode-children
    unicode_page = PageFactory(depth=3, path='000100010004', slug='unicode-children', numchild=3, show_in_menus=True)
    # /unicode-children/latin-capital-letter-i-with-diaeresis-Ï
    PageFactory(depth=4, path=f'{unicode_page.path}0001', slug='latin-capital-letter-i-with-diaeresis-Ï', show_in_menus=True)
    # /unicode-children/cyrillic-capital-letter-ya-Я
    PageFactory(depth=4, path=f'{unicode_page.path}0002', slug='cyrillic-capital-letter-ya-Я', show_in_menus=True)
    # /unicode-children/cjk-fire-火
    PageFactory(depth=4, path=f'{unicode_page.path}0003', slug='cjk-fire-火', show_in_menus=True)
